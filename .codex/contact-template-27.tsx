...
function normalizeInstantContacts(card) {
  return asArray(card?.instantContacts).filter((item) => item && text(item.label));
}

function normalizeSupportHighlights(value) {
  return asArray(value).filter((item) => item && (text(item.title) || text(item.body) || text(item.icon)));
}
...
function renderDialogActions(item, labels) {
  const copyValue = text(item.copyValue || item.value);
  const copyLabel = text(item.copyLabel || labels.copyLabel);
  const copyToastLabel = text(item.copyToastLabel || labels.copyToastLabel);
  const openLabel = text(item.openLabel || labels.openLabel);
  const href = text(item.href);
  const actions = [];
  if (copyValue && copyLabel) {
    actions.push(
      <button
        key="copy"
        className="sg-ui-ios-button sg-ui-ios-button--secondary sg-ui-dialog__footer-action"
        data-sg-ui-dialog-action-close="true"
        data-sg-ui-dialog-copy-text={copyValue}
        data-sg-ui-dialog-copy-toast-text={copyToastLabel || undefined}
        type="button"
      >
        {copyLabel}
      </button>
  ...
  if (href && openLabel) {
    actions.push(
      <a
        key="open"
        className="sg-ui-ios-button sg-ui-ios-button--primary sg-ui-dialog__footer-action"
        data-sg-ui-dialog-action-close="true"
        href={href}
...
function InstantContactItem({ item, labels, officeIndex, contactIndex }) {
  const type = text(item.type || item.iconType).toLowerCase();
  const symbolId = type ? `#sg-contact-icon-${type}` : '';
  const label = text(item.label);
  const ariaLabel = text(item.ariaLabel || label);
  const href = text(item.href);
  const qrImage = text(item.qrImage);
  const qrAlt = text(item.qrAlt);
  const value = text(item.value);
  const pendingText = text(item.pendingText);
  const dialogTitle = text(item.dialogTitle || label);
  const closeLabel = text(item.closeLabel || labels.closeLabel);
  const dialogId = `sg-ui-dialog-${officeIndex}-${contactIndex}`;
  const panelId = `${dialogId}-panel`;
  const isWecom = type === 'wecom';
  const wrapClassName = ['sg-instant-contact__action-wrap', isWecom ? 'sg-instant-contact__action-wrap--wecom' : ''].filter(Boolean).join(' ');
  const actionClassName = ['sg-instant-contact__action', 'sg-instant-contact__action--desktop-only', type ? `sg-instant-contact__action--${type}` : ''].filter(Boolean).join(' ');
  const mobileActionClassName = ['sg-instant-contact__action', 'sg-instant-contact__action--mobile-only', type ? `sg-instant-contact__action--${type}` : ''].filter(Boolean).join(' ');
  const actionContent = (
    <>
      <span aria-hidden="true" className="sg-instant-contact__icon">
        <svg className={`sg-instant-contact__symbol sg-instant-contact__symbol--${type || 'default'}`}>
  ...
  return (
    <div className={wrapClassName}>
      <div className="sg-ui-tooltip sg-ui-tooltip--top" data-sg-ui-tooltip>
        {href ? (
          <a
            aria-label={ariaLabel}
            className={`sg-ui-tooltip__trigger ${actionClassName}`}
            data-sg-ui-tooltip-trigger
            href={href}
            rel={item.external === false ? undefined : 'noreferrer'}
            target={item.external === false ? undefined : '_blank'}
          >
        ...
        <div aria-label={ariaLabel} className="sg-ui-tooltip__content sg-instant-contact__popover" role="tooltip">
          <div className={`sg-instant-contact__panel-content ${qrImage || pendingText ? 'sg-instant-contact__panel-content--with-qr' : ''}`.trim()}>
            {qrImage ? <img alt={qrAlt} className="sg-instant-contact__qr-image" src={qrImage} /> : null}
            {!qrImage && pendingText ? <div className="sg-instant-contact__qr-placeholder">{pendingText}</div> : null}
            {value ? <p className="sg-instant-contact__panel-detail">{value}</p> : null}
          </div>
        </div>
      </div>

      <div className="sg-ui-dialog" data-sg-ui-dialog data-sg-ui-dialog-mobile-only="true">
        <div className="sg-ui-dialog__anchor">
          <button
            aria-controls={panelId}
            aria-expanded="false"
            aria-haspopup="dialog"
            aria-label={ariaLabel}
            className={`sg-ui-dialog__trigger ${mobileActionClassName}`}
        ...
        <div className="sg-ui-dialog__root" data-sg-ui-dialog-root hidden>
          <button aria-label={closeLabel} className="sg-ui-dialog__backdrop" data-sg-ui-dialog-close tabIndex={-1} type="button"></button>
          <section aria-labelledby={`${panelId}-title`} aria-modal="true" className="sg-ui-dialog__panel sg-ui-dialog__panel--sm" id={panelId} role="dialog">
            <div className="sg-ui-dialog__header">
              <h2 className="sg-ui-dialog__title" id={`${panelId}-title`}>{dialogTitle}</h2>
              <button aria-label={closeLabel} className="sg-ui-dialog__close" data-sg-ui-dialog-close type="button">
            ...
            <div className="sg-ui-dialog__footer">
              <div className={['sg-ui-dialog__footer-actions', renderDialogActions(item, labels).length > 1 ? 'sg-ui-dialog__footer-actions--double' : ''].filter(Boolean).join(' ')}>
                {renderDialogActions(item, labels)}
              </div>
            </div>
          </section>
        </div>
...
function OfficeCard({ card, labels, officeIndex }) {
  const addressLines = asArray(card?.addressLines).filter((item) => text(item));
    ...
    <article className="sg-contact-highlight sg-contact-page__sidebar-office">
      <span aria-hidden="true" className="sg-contact-highlight__icon sg-contact-highlight__icon--glyph">
        <svg fill="none" viewBox="0 0 24 24">
          <path d={PHONE_ICON_PATH} stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
        </svg>
      </span>
      <div className="sg-contact-highlight__body">
        {text(card?.title) ? <strong>{card.title}</strong> : null}
        {addressLines.length ? (
          <address className={['office__address', 'sg-contact-page__sidebar-address', addressLines.length === 1 ? 'sg-contact-page__sidebar-address--single-line' : ''].filter(Boolean).join(' ')}>
...
export default function Template(props) {
  ...
  const content = (
    <main className="sg-page-shell sg-content-shell sg-contact-page">
      ...
      <div className="banner-primary" data-component="further-reduced-masthead" data-size="small">
        {heroImage ? <img alt={title} className="banner-primary__image" fetchPriority="high" src={heroImage} /> : null}
        <div aria-hidden="true" className="banner-primary__overlay"></div>
        <div className="wrapper">
          <div className="banner-primary__content">
            <div className="banner-primary__holder">
              {title ? <h1 className="banner-primary__title">{title}</h1> : null}
              {heroSummary ? <p className="banner-primary__copy">{heroSummary}</p> : null}
            </div>
          </div>
        </div>
      </div>
