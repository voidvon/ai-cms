...
- 所有正文 `<img>` 必须同时包含数值型 `width` 和 `height` 属性。先根据 PDF 中的视觉占比、所在栏宽和图片用途确定适合版面的显示尺寸，再把 `assets/` 中交付的最终位图物理缩放到相同像素尺寸；HTML 属性必须与最终文件的真实像素宽高完全一致。2x、3x 高倍率图只能作为转换中间文件，除非用户明确要求高密度资源，否则不得直接作为最终图片。禁止猜测比例、写百分比或只写单边尺寸。
...
- 对复杂图形使用足够高的临时渲染倍率，通常从 3 到 4 倍开始完成裁切和清理，再将最终交付位图缩放到确定的显示尺寸。
- 图片导出后先确定适合版面的显示宽高，再将最终位图物理缩放到该像素尺寸，并把相同数值写入 HTML 的 `width`、`height`。工程图、局部裁图和小型示意图应使用与正文关系协调的尺寸，不能因为中间文件像素较大就铺满整栏；不得沿用导出前、中间文件或 PDF 页面截图的尺寸。
- “文件像素与 HTML 属性一致”只是尺寸校验，不代表尺寸选择正确。必须根据每张图的用途、所在单栏或双栏、PDF 原始视觉占比、标注可读性及周围内容密度逐图确定合适尺寸；禁止把原始导出尺寸直接作为显示尺寸，也禁止给同一文档中的所有图片机械套用同一宽度。产品总成图、曲线图、部件标注图和尺寸工程图应分别判断。
...
- 图片基础样式必须保持 `width: auto; max-width: 100%; height: auto`，让 HTML 尺寸属性决定默认显示尺寸，并只在可用宽度不足时等比例缩小。不得用通用 `width: 100%` 把小图、工程图或裁切图自动放大到容器宽度；确需更大展示时使用已有语义类和可验证的 `max-width`，不要移除尺寸属性。
- 将移动端断点限定为屏幕介质，例如：
...
- 检查每个 `<img>` 都有正整数 `width`、`height`，并逐图确认属性值与最终位图文件的真实像素宽高完全一致；确认没有图片因通用 CSS 超过其声明宽度被放大，窄屏时仍能等比例缩小。
- 必须在桌面、手机和 A4 打印渲染中逐图检查视觉尺寸。图片明显压过相邻正文、挤占主要版面、产生不合理大块留白，或缩小到标注难以辨认，都视为未完成；应重新选择合适尺寸，同时物理缩放最终位图并更新 HTML 属性，直到文件尺寸、HTML 尺寸和实际版面三者一致。
