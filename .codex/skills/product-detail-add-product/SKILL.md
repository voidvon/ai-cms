...
description: 在当前数据库驱动的 Spirax Sarco Node.js CMS 中筛选、新增或更新产品详情内容，从 docs/价格汇总表.csv 提取产品规格，接入既有产品栏目树、product 内容模型、数据库 TSX 模板、产品图片、多语言翻译和自然站内链接，并完成静态生成验证。适用于新增商品/产品详情页、补建缺失型号、维护产品规格与询盘控件、优化多语言产品内链、添加多语言内容、设置产品主图或首页推荐、检查产品栏目归属与静态 URL 等任务。
---
...
3. 目标语言为 `zh-CN` 时，英文文档必须先转换为简体中文再合并；保留型号、文档编号、标准、表格数值、单位、技术条件和安全含义，不得把英文正文直接混入中文版。
...
- 从 `docs/关键词列表/按产品系列类型拆分/*.csv` 获取搜索表达和已有内链目标，但必须确认目标静态页或已发布内容真实存在；关键词表中的失效、未生成或仅规划 URL 不得写入正文。
- 先审查默认语言正文的链接目标、出现语境和信息层级。其他语言默认复用这套科学的信息架构，只本地化锚文本和语言路径；不要为了增加数量额外堆叠相关型号。
...
1. 重新读取条目并核对栏目、所有目标语言、发布状态、图片、SEO 和规格字段；逐项回查 `docs/价格汇总表.csv`。
2. 用 `buildContentDetailPathFromColumn` 对应的现有规则确认预期 URL，检查没有与既有内容冲突。
3. 完成全部数据库、模板和翻译修改后再运行一次 `npm run build:site`。构建期间不要继续修改真源，也不要并发启动第二个静态构建；如通过 MCP 触发构建，显式指定目标语言，不依赖工具的默认 EN 行为。
4. 通过公开路由抽查首页 `/`、联系栏目 `/contact-us/`、新增产品详情页、所属产品列表页和一个新闻详情页。
5. 检查列表页能从栏目树进入新产品、详情模板为数据库发布态 TSX 模板、图片存在且无旧路径、目标语言未错误回退；分别抽查有规格和无规格产品，确认数量选择与联系按钮固定存在。对多语言内链逐项检查目标存在、语言前缀、锚文本、边界空格，并确认没有 `nofollow` 或新窗口属性。
6. 更新已有正文或导入文档后，检查原产品简介仍是第一段可见正文，并核对销售手册、技术资料、安装维修指南的出现位置符合既定优先级；目标语言为 `zh-CN` 时检查不存在非必要的英文正文残留。
7. 报告条目 ID、栏目、最终 URL、发布语言、首页推荐状态、图片状态和构建结果。
