...
先按 [产品主题与关键词研究](keyword-research.md) 固定页面覆盖型号，并完成整个关键词拆分目录的搜索、去重和四种统计口径。多型号资料不自动扩大产品页范围。
...
```json
{
  ...
  "translations": {
    "en": {
      "name": "TD52 Thermodynamic Steam Trap",
      "summary": "A concise, verifiable English product description.",
      "content_html": "<p>Verified English product content.</p>",
      "template_data_json": null,
      "seo_title": "Spirax Sarco TD52 Thermodynamic Steam Trap",
      "seo_description": "An accurate English search description.",
...
示例中的栏目 ID、图片、规格和文案仅展示结构，不能直接复用为事实。`zh-CN` 仅演示统一内容服务因默认语言必填而需要的技术性草稿壳；它不能包含英文占位内容，也不能发布。
...
- 产品内容流程以 `en` 为母版语言，与数据库默认语言无关。先完成并验收英文；只有用户在英文完成后明确确认，才扩展其他启用语言。
- 数据库默认语言通常是 `zh-CN`，但仍应读取 `languages.is_default` 确认。新建条目若因服务约束必须提供默认语言记录，只创建“型号名称 + 空正文 + draft”的草稿壳。
- 同一产品的多语言版本写入一个条目的 `translations`，不创建多个产品文件或重复基础记录。
- 未完成的语言不设 `published`；验证页面时检查 `resolved_language_code` 和 `is_language_fallback`。
- 阶段 A 只写入并发布英文母版。阶段 B 重新读取已验收的英文，再为每种启用语言独立本地化 `name`、`summary`、正文和 SEO 字段，不从其他译文转译，也不机械翻译技术术语。
- 关键词资料只指导搜索表达，产品事实必须来自技术资料。
- 初始请求中的“全部语言”不构成越过英文验收确认门槛的授权。

### 多语言正文内链

内链优化按“英文母版信息架构 + 各语言自然表达”执行：

1. 从已验收的英文正文提取正文站内 URL、锚文本和上下文，不把 `#section` 页内目录与正文跨页链接混为一类。
2. 从 `docs/关键词列表/按产品系列类型拆分/*.csv` 找与当前产品、父分类和使用场景直接相关的词及其 `内链` 列。
3. 对候选 URL 检查数据库发布状态和实际静态文件。类似关键词表中存在但站点未生成的 `/topics/.../` 不得使用。
4. 其他语言原则上复用英文母版的链接目标和语义位置。本地化锚文本时优先采用目标栏目/内容的已发布翻译名称，再结合关键词表达调整为句内自然形式。
5. 只有能明确帮助选型且目标页已发布时才增加英文母版没有的相关产品链接；不要以链接数量作为优化目标。
...
建议分别统计每种语言：正文跨页链接数、页内锚点数、带 `rel/target` 的站内链接数、目标不存在数和锚点边界异常数。已验收英文母版不应因“同步多语言”被无意修改。
...
- 对 `code`、URL、所有目标语言标题/SEO/正文、图片替代文本、图注和询盘规格执行产品主题审计；单型号页的身份与转化字段中排除型号出现数必须为 `0`，正文未审查或误导性兄弟型号引用数必须为 `0`。合理比较、兼容性、选型分流或必要来源说明应列入批准例外清单。
- 分别报告关键词目录原始命中数、匹配行/文件数、主 CSV 去重有效查询数和唯一可用内链目标数。
- 核对每个翻译的名称、SEO、正文和发布状态。
- 对每个已发布语言核对正文内链目标、语言前缀、本地化锚文本、边界空格和标点；站内链接的 `nofollow/target` 数必须为 `0`。
- 再次搜索型号，确认只有一个基础条目。
- 更新 `custom_url` 或撤回语言时核对旧静态路径；不得把数据库已改为草稿误报为旧静态文件已经清理。
...
英文公开页验收通过后必须停止并请求用户确认。收到确认、完成全部启用语言后，再执行完整静态生成：
...
英文构建是阶段 A 的最后一步，完整构建是阶段 B 的最后一步。若构建被交互中断但进程仍在后台运行，先检查并等待该进程结束；不要在旧构建尚未结束时修改数据后再启动新构建，否则不同语言目录可能混入不同版本的真源。
