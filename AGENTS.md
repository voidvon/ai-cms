...
- 模板源码默认按三个槽位理解和维护：
  ...
  - `css_source`：当前模板的局部样式
  - `global_css_source`：进入全站公共样式包的全局样式
- `templates` 与 `template_versions` 已完成结构迁移；旧字段 `content`、`published_content` 不再作为模板存储结构的一部分，新增代码禁止恢复、回写或依赖这套旧字段。
- 不要再把 `export const scss` / `export const css` 当成数据库模板存储规范；模板真实数据只能写入 `tsx_source / css_source / global_css_source`，发布态只能写入对应的 `published_*` 字段。
- 如果遇到历史模板源码中仍内嵌样式，应视为一次性迁移输入，而不是长期兼容格式；迁移完成后应直接删除兼容路径，而不是继续保留双写或回退逻辑。
- 页面级 CSS 与全站级 CSS 要明确区分：
  - `css_source` 只进入页面级 `page-*.css`
  - `global_css_source` 只进入全站级 `shared.css`
- 壳层、导航、页脚、全局搜索、全站复用基础 UI 等样式，优先进入 `global_css_source`，不要继续混在页面模板或详情模板样式里。
- 组件是否“全局”不由系统预判；只要该组件样式应随整站加载，就应写入该模板的 `global_css_source`。
- 修改模板导入脚本、模板管理后台、静态构建器、模板预览链路时，必须保证这三个槽位在保存、发布、预览、历史版本、静态生成上的语义一致。
- 如果需要从旧模板迁移样式，优先把真正的站点公共样式提取到 `global_css_source`，不要只是机械地把旧样式包装后塞回新结构。
