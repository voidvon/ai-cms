...
一级分类：   /products/{id}.html
二级分类：   /products/{id}.html
产品详情：   /products/{category-slug}/{product-slug}/index.html
```

### 修改后
```
产品根页：   /products/index.html
一级分类：   /products/{category-slug}/index.html
二级分类：   /products/{parent-slug}/{category-slug}/index.html
产品详情：   /products/{parent-slug}/{category-slug}/{product-slug}/index.html
```
...
**修改点1**: 传递 `categoryMap` 给 `writeProductCategoryPageSet`

**位置**: `system/server/src/static-builder.mjs` 第367-399行

在调用 `writeProductCategoryPageSet` 时添加 `categoryMap` 参数。

**修改点2**: `writeProductCategoryPageSet` 函数使用完整slug路径

**位置**: `system/server/src/static-builder.mjs` 第1871-1919行

- 接收 `categoryMap` 参数
- 使用 `buildCategorySlugPath` 构建完整路径
- 输出路径改为 `/products/{parent-slug}/{category-slug}/index.html`
- 保留旧的数字ID路径作为兼容性备份
...
修改所有生成分类URL的函数，传递 `categoryMap` 参数以支持完整路径生成。

#### 1. 修改核心URL生成函数

**`buildLegacyProductCategoryUrl` 函数**：
```javascript
...
#### 3. 传递categoryMap到所有相关函数

- `buildLegacyProductNavigation({ ..., categoryMap })`
- `buildLegacyProductMenuItems(categories, activeId, categoryMap)`
- `buildLegacyProductBreadcrumbItems(category, parent, categoryMap)`
...
产品根页（`category.id = 0`）是一个虚拟分类，没有 `slug` 字段。因此 `useSlugPath` 判断为 `false`，走了旧的数字ID逻辑，生成了 `index-1.html`、`index-2.html`、`index-3.html` 等分页文件，而不是 `index.html`。

### 解决方案

修改 `writeProductCategoryPageSet` 函数中的判断逻辑：

## 第四阶段：修复子分类卡片链接 (2026-06-14)

### 问题描述

在完成前三阶段后，尽管静态页面的文件路径和侧边栏链接都使用了完整的层级结构，但一级分类页面（如 `/products/flowmetering/`）中显示的**子分类卡片**链接仍然是错误的短路径：
...
问题出在从 spirax-global 项目导入的 `legacy_extra` 数据中。这些一级分类的 `page_data.cards` 包含了硬编码的旧URL：
...
在 `buildLegacyProductListPageProps` 函数中，修正 `categoryPageData.cards` 中的 URL，使用 `buildLegacyProductCategoryUrl` 生成完整的层级路径：
...
通过三种方式匹配 `pageData.cards` 中的卡片与实际的子分类：

1. **名称匹配**：`card.title === child.name`
2. **ID匹配**：`card.link` 包含 `/${child.id}.html`
3. **Slug匹配**：`card.link` 包含 `/${child.slug}`

匹配成功后，使用 `buildLegacyProductCategoryUrl(matchingChild, categoryMap)` 生成正确的完整层级URL。
...
function buildProductUrl(product, categorySlug = null) {
  ...
  if (product.slug && categorySlug) {
    return `/products/${categorySlug}/${product.slug}/`;  // ❌ 只用单层slug
  }
  // ...
}
```
...
function setGlobalCategorySlugMap(categories) {
  ...
  globalCategorySlugMap = new Map(
    categories.map(cat => {
      const slugPath = buildCategorySlugPath(cat, globalCategoryMap);  // ✅ 构建完整路径
...
function buildProductUrl(product, categorySlugPath = null) {
  ...
  if (product.slug && categorySlugPath) {
    return `/products/${categorySlugPath}/${product.slug}/`;  // ✅ 使用完整路径
  }
  // ...
}
```
...
关键改进：
1. 新增 `globalCategoryMap`：存储分类ID到完整分类对象的映射
2. 使用 `buildCategorySlugPath(cat, globalCategoryMap)`：向上遍历构建完整的slug路径数组
...
问题出在 `writeProductCategoryPageSet` 函数调用 `buildLegacyProductListPageProps` 时，**没有传递 `categoryMap` 参数**。

虽然 `buildLegacyProductListPageProps` 函数内部会创建一个 `categoryMap`，但这是在**调用之后**才创建的。而在 `writeProductCategoryPageSet` 中已经有了一个现成的 `categoryMap`，应该直接传递过去，避免重复创建。

更关键的是，如果不传递参数，函数签名中就没有这个参数的定义，导致在某些边界情况下（如产品根页）可能出现问题。

### 解决方案

#### 1. 修改函数签名，添加 `categoryMap` 参数
...
function buildLegacyProductListPageProps({ templateContext, category, parent, children, pageItems, pageNumber, pageCount, totalRecords }) {
  // ...
  const categoryMap = new Map(templateContext.productCategories.map(...));
}
...
function buildLegacyProductListPageProps({ templateContext, category, parent, children, pageItems, pageNumber, pageCount, totalRecords, categoryMap = null }) {
  ...
  if (!categoryMap) {
    categoryMap = new Map(templateContext.productCategories.map(...));
  }
}
```

#### 2. 在调用时传递 `categoryMap`
...
所有一级分类链接都使用完整的slug格式，所有URL返回 HTTP 200 ✅
...
**产品分类页（bellows-sealed-stop-valves）**：
```html
...
| 阶段 | 问题 | 解决方案 | 受益范围 |
|------|------|----------|----------|
| 1 | 静态文件路径单层 | `buildCategorySlugPath` 递归构建完整路径 | 文件系统 |
| 2 | 侧边栏链接用ID | `buildLegacyProductCategoryUrl` 传递 categoryMap | 导航菜单 |
| 3 | 产品根页404 | 特殊处理 categoryId === 0 | 根页面 |
| 4 | 子分类卡片硬编码 | 修正 `page_data.cards` | 分类页卡片 |
| 5 | 产品详情缺父级 | `globalCategorySlugMap` 存储完整路径 | 产品链接 |
| 6 | 根页侧边栏用ID | 传递 categoryMap 到 props 函数 | 根页侧边栏 |
...
function buildLegacyProductCategoryUrl(category, categoryMap = null) {
  ...
  // 最后回退到ID格式
  return `/products/${category.id}.html`;
}
```
...
const categorySlugPath = category.slug && categoryMap
  ...
  : [];
const useSlugPath = categorySlugPath.length > 0;
...
const categorySlugPath = category.slug && categoryMap
  ? buildCategorySlugPath(category, categoryMap)
  : [];
const useSlugPath = categorySlugPath.length > 0 || categoryId === 0;
// 根分类也使用新路径格式
```
...
if (useSlugPath) {
  outputDir = categorySlugPath.length > 0
    ? path.join('products', ...categorySlugPath)  // 子分类：/products/{path}/
    : 'products';                                  // 根分类：/products/
