...
<tr><td height=25>
	...
	<td>
		...
		<tr>
		<td class=StatusBarBtnOff id=eWebEditor_CODE onClick="setMode('CODE')"><img border=0 src="buttonimage/<%=sStyleDir%>/html.gif" width=59 height=20 align=absmiddle></td>
		<td width=5></td>
		<td class=StatusBarBtnOff id=eWebEditor_EDIT onClick="setMode('EDIT')"><img border=0 src="buttonimage/<%=sStyleDir%>/Editor.gif" width=59 height=20 align=absmiddle></td>
		<td width=5></td>
		<td class=StatusBarBtnOff id=eWebEditor_TEXT onClick="setMode('TEXT')"><img border=0 src="buttonimage/<%=sStyleDir%>/Text.gif" width=59 height=20 align=absmiddle></td>
		<td width=5></td>
		<td class=StatusBarBtnOff id=eWebEditor_VIEW onClick="setMode('VIEW')"><img border=0 src="buttonimage/<%=sStyleDir%>/browse.gif" width=59 height=20 align=absmiddle></td>
		</tr>
		</table>
	</td>
	...
	<td align=right>
		<table border=0 cellpadding=0 cellspacing=0 height=20>
		<tr>
		<td width="34" style="cursor:pointer;" onClick="sizeChange(300)"><div align="right"><img border=0 SRC="images/plus.gif" width=20 height=20 alt="减小编辑区"></div></td>
		<td width=19></td>
		<td width="27" style="cursor:pointer;" onClick="sizeChange(-300)"><img border=0 SRC="images/minus.gif" width=20 height=20 alt="增高编辑区"></td>
		<td width=10></td>
		</tr>
		</table>
	</td>
...
Sub Init()
	...
	sSql = "select * from Yixiang_ewebeditor_style where s_name='" & sStyleName & "'"
	...
	' 取版本号及发布日期
	sSql = "select sys_version,sys_releasedate,sys_license from Yixiang_ewebeditor_system"
	oRs.Open sSql, oConn, 0, 1
	sVersion = oRs(0)
	sReleaseDate = oRs(1)
	sLicense = oRs(2)
...
Sub InitButtonArray()
	Dim i
	sSql = "select * from Yixiang_ewebeditor_button order by b_order asc"
	oRs.Open sSql, oConn, 0, 1
	i = 0
...
Sub InitToolBar()
	Dim aButton, n
	sSql = "select t_button from Yixiang_ewebeditor_toolbar where s_id=" & sStyleID & " order by t_order asc"
	oRs.Open sSql, oConn, 0, 1
	If Not oRs.Eof Then
