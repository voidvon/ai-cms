...
Sub Main()
 ...
 %>
	<ul style="margin:0px;text-align: left;width:100%;"> 
     <form name="myform" method="post" action="upload.asp?t=1&tMode=<%=tMode%>&istwo=0&utype=<%=utype%>&guser=<%=guser%>" enctype="multipart/form-data">
    ...
    upfiletype=replace(upfiletype,"|",",")
   	...
   	Set Upload = New UpFile_Cls
		...
		If Upload.ErrCodes<>0 Then
 			Response.write "错误："& Upload.Description & "[ <a href='upload.asp?tMode="&tMode&"&istwo=1&utype="&utype&"&guser="&guser&"'>重新上传</a> ]"
			Exit Sub
		End If
		If Upload.Count > 0 Then
			...
			Next
		Else
			Response.write "请正确选择要上传的文件。[ <a href='upload.asp?tMode=" & tMode &"'>重新上传</a> ]"
			Exit Sub
		End If
	Set Upload = Nothing
 End Sub
