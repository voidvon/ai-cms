...
Sub Main()
 ...
 %>
	<ul style="margin:0px;text-align: left;width:100%;"> 
     <form name="myform" method="post" action="upload3.asp?t=1&tMode=<%=tMode%>&istwo=0&utype=<%=utype%>&guser=<%=guser%>" enctype="multipart/form-data">
    ...
    upfiletype=replace(upfiletype,"|",",")
   	...
   	Set Upload = New UpFile_Cls
		...
		If Upload.ErrCodes<>0 Then
 			Response.write "错误："& Upload.Description & "[ <a href='upload3.asp?tMode="&tMode&"&istwo=1&utype="&utype&"&guser="&guser&"'>重新上传</a> ]"
			Exit Sub
		End If
		If Upload.Count > 0 Then
			For Each FormName In Upload.UploadFiles
				...
				F_FileName = FilePath & File.FileName
					...
					Response.Write "大图上传成功!  "&hgc
					response.write "<script>parent.form.magicfacepic2.value='/UploadFile/produppic/"&File.FileName&"'</script>"

				Set File = Nothing
			Next
		Else
			Response.write "请正确选择要上传的文件。[ <a href='upload3.asp?tMode=" & tMode &"'>重新上传</a> ]"
			Exit Sub
		End If
	Set Upload = Nothing
 End Sub
