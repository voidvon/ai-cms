...
dim ishavegant
 ...
 Rs.Open ("select news_detail from benming_ch_worldec_Temp where selected=1"),conn,1,1
 If Not Rs.Eof Then 
 	templets=Rs("news_detail")
	Rs.Close
	Set Rs=nothing
End If
...
End If
       ...
       '^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
		...
		RsNews.Open ("Select * from benming_ch_news where Typeid in("&Getid(4)&") and newsid between "&id1&" and "&id2&""),conn,1,1
		...
		Set sort_save = fso.CreateTextFile(server.mappath("/news/detail/"&pageName&".html"))
