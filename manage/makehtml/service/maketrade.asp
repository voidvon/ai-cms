...
Rs.Open ("select service_sort1 from benming_ch_worldec_Temp where selected=1"),conn,1,1
If Not Rs.Eof Then 
	templets=Rs("service_sort1")
	Rs.Close
	Set Rs=nothing
End If
...
End If
		...
		'^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^开始生成html页面
		MaxPerPage=1
		Sql="Select * from benming_ch_NewsCat where root=12 order by ORderID"
				...
				if mpage2>=1 then
					for tempii=1 to mpage2
						...
						do while not RsNewsCount2.eof
            				...
            				Hope_body=Hope_body&"</tr>"
            				Hope_body=Hope_body&"<tr>"
              				Hope_body=Hope_body&"<td height=""50"" colspan=""2"" valign=""middle"" class=""news_bottom_line news_sp 0a"" >"&gotTopic(RsNewsCount2("desc"),230)&"</td>"
            				Hope_body=Hope_body&"</tr>"
						...
						if currentPage=1 and tempii=1 then
							Set sort_save = fso.CreateTextFile(server.mappath("/service/index.html"))
							sort_save.Write pencat
							sort_save.Close
						end if
						
						if tempii=1 then
							Set sort_save = fso.CreateTextFile(server.mappath("/service/"&pageName&"-1.html"))
							sort_save.Write pencat
							sort_save.Close
						end if
						
						Set sort_save = fso.CreateTextFile(server.mappath("/service/"&pageName&".html"))
						sort_save.Write pencat
						sort_save.Close
