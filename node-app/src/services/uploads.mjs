...
import { PROJECT_ROOT, UPLOAD_ALLOWED_EXTENSIONS, UPLOAD_MAX_SIZE_KB } from '../config.mjs';
...
export function saveUploadedFile(file, options = {}) {
  ...
  const fsDir = path.join(PROJECT_ROOT, target.fsDir);
...
function resolveDirectUploadCandidates(normalized) {
  const uploadsRoot = path.resolve(PROJECT_ROOT, 'uploadfile');
  const stripped = normalized.replace(/^\/+/, '');
  const segments = stripped.split('/').filter(Boolean);

  if (segments.length === 1 && /\.[a-z0-9]+$/i.test(segments[0])) {
    return [
      path.resolve(PROJECT_ROOT, 'uploadfile/newsuppic', segments[0]),
      path.resolve(PROJECT_ROOT, 'uploadfile/produppic', segments[0])
    ].filter((filePath) => isInsideUploadsRoot(filePath, uploadsRoot));
  }

  if (segments.length >= 2 && segments[0].toLowerCase() === 'aboutuppic') {
    return [
      path.resolve(PROJECT_ROOT, 'uploadfile/newsuppic', segments.slice(1).join('/'))
    ].filter((filePath) => isInsideUploadsRoot(filePath, uploadsRoot));
  }
  ...
  const filePath = path.resolve(PROJECT_ROOT, segments.join('/'));
