...
if request.querystring("id")="" then
	...
	Sql="Select * from benming_ch_ProdCat where root="&request.querystring("id")&"  order by orderid"
end if
...
<Form name="search" method="POST" action="index.asp">
  <table wnewsidth="100%" border="0" align="center" cellpadding="2" cellspacing="1" class="tableBorder">
    <tr>
      <th class="tableHeaderText" height=25 colspan="4">产品分类列表</th>
    <tr>
      <td colspan="4">      </td>
    </tr>
...
%> 
	 <tr height="20">
	  ...
	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="prodcat.asp?id=<%=Rs("id")%>"><%=Rs("CatName")%></a>[<span class="STYLE1"><%=GetCount(Rs("id"))%></span>]</td>
      <td align="left" class=forumRow>&nbsp;
	  <%if rs("root")=0 then 
	  		response.write("一级分类")
		else
      ...
      <td height="30" align="center" class=forumRow><a href="prodcat_edit.asp?id=<%=Rs("id")%>">修改</a> | <a href="prodcat_save.asp?action=del&id=<%=Rs("id")%>">删除</a>
      </td>
    </tr>
    <%
	rowcount=rowcount-1
...
<tr height="20" bgcolor="#ffffff">
      <td height="30" colspan="4" align="center" class=forumRow> <%=listPages("prodcat.asp?id="&request.querystring("id"))%></td>
    </tr>
    <tr height="20" bgcolor="#ffffff">
