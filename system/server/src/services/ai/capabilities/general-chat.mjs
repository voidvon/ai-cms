...
export const generalChatCapability = {
  ...
  icon: '💬',
  category: 'general',
  visibleToolNames: ['query_columns', 'query_content_items', 'get_content_item_translation', 'price_lookup', 'fetch_url'],
  ...
  selectTools: (context) => {
    ...
    if (context.user) {
      if (context.user.hasPermissions(['read:content'])) {
        tools.push('query_columns', 'query_content_items', 'get_content_item_translation');
      }
    }
